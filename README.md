...
```sh
brew uninstall --cask flashcard        # remove the app
brew uninstall --cask --zap flashcard  # remove the app and all its data
```

### Direct download

Grab the `.dmg` or `.zip` from the [releases](https://github.com/allisterk2703/flashcard/releases/latest) and drag **FlashCard.app** into `/Applications`.

Uninstall:

```sh
rm -rf /Applications/FlashCard.app
rm -rf ~/Library/Application\ Support/FlashCard   # app data (optional)
```

### From source

```sh
git clone https://github.com/allisterk2703/flashcard.git
cd flashcard
make install && make run
```

Uninstall: delete the cloned folder (and the app data folder above if you want a full cleanup).
